...
export default function ClaimChatBox({ claimId, claim }) {
    const { 
        messages, 
        isLoading, 
        typingUser, 
                ...
                <div className={`max-w-[70%] ${isMe ? 'text-right' : ''}`}>
                    ...
                    {msgType === 'IMAGE' && (
                        <img 
                            ...
                            onClick={() => window.open(message.metadata?.imageUrl || message.content, '_blank')}
                        />
                    )}
                    ...
                    {msgType === 'LOCATION' && (
                        ...
                        `}>
                            ...
                            <a 
                                href={`https://www.google.com/maps?q=${message.metadata?.location?.coordinates?.lat},${message.metadata?.location?.coordinates?.lng}`}
                                target="_blank"
                    ...
                    {msgType === 'MEETUP_PROPOSAL' && (
                        ...
                        `}>
                            ...
                            <div className="space-y-1 text-sm">
                                {message.metadata?.meetup?.time && (
                                    <p><strong>When:</strong> {format(new Date(message.metadata.meetup.time), 'MMM d, yyyy h:mm a')}</p>
                                )}
                                <p><strong>Location:</strong> {message.metadata?.meetup?.location}</p>
                            </div>
                            {!isMe && message.meetupStatus === 'PENDING' && (
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="font-medium text-sm truncate">
                                {claim.post?.title || 'Lost & Found Item'}
                            </p>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.length === 0 ? (
                    <div className="text-center text-gray-500 py-8">
                        <p>No messages yet.</p>
                        <p className="text-sm mt-1">Start the conversation to coordinate the claim!</p>
                    </div>
                ) : (
